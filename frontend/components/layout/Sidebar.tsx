...
export const Sidebar: React.FC<SidebarProps> = ({ mobileOpen = false, setMobileOpen }) => {
  ...
  // Mock Role - In production, this comes from authUser claims
  ...
  const SidebarContent = ({ isMobile = false }) => (
    <div className="flex flex-col h-full bg-[#0A0E17] border-r border-white/5">
      {/* Brand */}
      <div className="p-6 flex items-center gap-3 border-b border-white/5 bg-[#0A0E17]/50 backdrop-blur-xl">
      ...
      <div className="p-4 border-t border-white/5 bg-[#080b12]">
        <div className="bg-white/5 rounded-xl p-3 flex items-center gap-3 border border-white/5 hover:border-white/10 transition-colors group cursor-pointer">
          <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-emerald-500 to-cyan-500 flex items-center justify-center text-xs font-bold text-white shadow-lg ring-2 ring-[#0A0E17] group-hover:ring-emerald-500/50 transition-all">
            JD
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-white truncate group-hover:text-emerald-400 transition-colors">John Doe</p>
            <p className="text-xs text-gray-500 truncate group-hover:text-gray-400">Senior Analyst</p>
          </div>
