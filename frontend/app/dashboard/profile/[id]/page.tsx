...
const styles = StyleSheet.create({
  ...
  header: { marginBottom: 20, borderBottomWidth: 2, borderBottomColor: '#10B981', paddingBottom: 10 },
  ...
  bold: { fontSize: 10, fontWeight: 'bold', color: '#111827' },
});

// PDF Component
const ProfilePDFDocument = ({ user, history }: { user: UserProfile, history: Transaction[] }) => (
  <Document>
    <Page size="A4" style={styles.page}>
      <View style={styles.header}>
        <Text style={styles.title}>Customer 360 Profile</Text>
        <Text style={styles.subtitle}>Generated by CloverShield • {new Date().toLocaleDateString()}</Text>
      </View>

      <View style={{ marginBottom: 20 }}>
        <View style={styles.row}><Text style={styles.label}>User ID:</Text><Text style={styles.value}>{user.user_id}</Text></View>
        <View style={styles.row}><Text style={styles.label}>Name:</Text><Text style={styles.value}>{user.name_en}</Text></View>
        <View style={styles.row}><Text style={styles.label}>Risk Level:</Text><Text style={{ ...styles.value, color: user.risk_level === 'high' ? '#EF4444' : '#10B981' }}>{user.risk_level?.toUpperCase()}</Text></View>
        <View style={styles.row}><Text style={styles.label}>Current Balance:</Text><Text style={styles.value}>৳ {user.balance?.toLocaleString()}</Text></View>
      </View>

      <Text style={{ fontSize: 14, fontWeight: 'bold', marginBottom: 10 }}>Recent Activity</Text>
      <View style={styles.tableHeader}>
        <Text style={[styles.text, styles.col1]}>Date</Text>
        <Text style={[styles.text, styles.col2]}>Type</Text>
        <Text style={[styles.text, styles.col3]}>Status</Text>
        <Text style={[styles.text, styles.col4]}>Amount</Text>
      </View>
      {history.slice(0, 50).map((tx, i) => (
        <View key={i} style={styles.tableRow}>
          <Text style={[styles.text, styles.col1]}>{tx.created_at ? new Date(tx.created_at).toLocaleDateString() : 'N/A'}</Text>
          <Text style={[styles.text, styles.col2]}>{tx.transaction_type}</Text>
          <Text style={[styles.text, styles.col3]}>{tx.status}</Text>
          <Text style={[styles.bold, styles.col4]}>৳ {tx.amount?.toLocaleString()}</Text>
        </View>
      ))}
    </Page>
  </Document>
  ...
  return (
    <div className={`space-y-8 pb-12 ${language === 'bn' ? 'font-bengali' : ''}`}>
      ...
      <div className="flex flex-col md:flex-row gap-6 pb-6 border-b border-white/10">
        <div className="flex-1 bg-card-bg border border-white/10 rounded-xl p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-3 bg-blue-500/10 rounded-lg text-blue-400">
              <Icon name="picture_as_pdf" />
            </div>
            <div>
              <h3 className="font-bold text-white">Profile Dossier</h3>
              <p className="text-xs text-text-secondary">Export comprehensive profile</p>
            </div>
          </div>
          <PDFDownloadLink
            document={<ProfilePDFDocument user={user} history={history} />}
            fileName={`profile-${user.user_id}.pdf`}
          ...
          >
            {({ loading }) => (loading ? 'Generating...' : <><Icon name="download" size={16} /> Export PDF</>)}
          </PDFDownloadLink>
        </div>
        ...
        <div className="flex-1 bg-card-bg border border-white/10 rounded-xl p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-white">Secure Transfer</h3>
              <p className="text-xs text-text-secondary">Air-gap data handoff</p>
            </div>
          </div>
            ...
            <QRDataBridge
              data={{
                ...
                user: {
                  ...
                  status: user.kyc_complete ? 'VERIFIED' : 'UNVERIFIED'
                },
                activity_log: history.slice(0, 50).map(tx => ({
