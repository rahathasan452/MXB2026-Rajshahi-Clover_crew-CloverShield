...
try:
    ...
    st.error("Configuration files not found. Please ensure config.py and mock_data.py are in the same directory as app.py")
    st.stop()
...
try:
    ...
    st.warning("joblib not installed. Running in demo mode without ML model.")
...
try:
    ...
    st.warning("Inference module not available. Using basic ML mode.")
...
def load_custom_css():
    ...
    /* Ensure columns display horizontally */
    [data-testid="column"] {{
        display: flex;
    ...
    try:
        ...
        if INFERENCE_AVAILABLE:
            try:
                ...
                st.sidebar.warning(f"Inference engine: {str(e)}")
            except Exception as e:
                st.sidebar.warning(f"Inference engine error: {str(e)}")
        ...
        st.sidebar.warning("ML model not found. Using rule-based detection.")
    except Exception as e:
...
def render_user_card(user_data, title):
    ...
    with col2:
        verified_icon = "Yes" if user_data['verified'] else "No"
        kyc_icon = "Yes" if user_data['kyc_complete'] else "No"
